...
const getDefaultColumns = (reportType: string): ColumnConfig[] => {
  ...
  if (reportType === 'CLOSE-OUT') {
    return [
      ...
      { key: 'formType', label: 'Form Type', visible: true, width: 100 },
      { key: 'programId', label: 'Program', visible: true, width: 100 },
      { key: 'assignmentDate', label: 'Assignment Date', visible: true, width: 100 },
      // { key: 'totalTimeSpentStr', label: 'Total Hours Worked', visible: true, width: 150 },
      // { key: 'totalVolume', label: 'Total Volume Worked', visible: true, width: 150 },
...
export default function BaseReport({ 
  ...
  const [selectedDate, setSelectedDate] = useState<string>(() => {
    ...
    // For 1341 report, default to today's date
    // For all other reports, default to yesterday's date
    const date = new Date();
    if (reportType !== '1341') {
  ...
  const [selectedEndDate, setSelectedEndDate] = useState<string>(() => {
    // For reports that support end date, default to same date as start date
    if (REPORTS_WITH_END_DATE.includes(reportType as any)) {
      const date = new Date();
      // For 1341 report, use today's date; for others, use yesterday's date
      if (reportType !== '1341') {
        date.setDate(date.getDate() - 1);
      }
      const month = (date.getMonth() + 1).toString().padStart(2, '0');
              ...
              <div className="w-48">
                ...
                >
                  {serviceCenterOptions.map((option) => (
                    <option key={option} value={option === 'Kansas City' ? 'kansas' : option}>
                      {option}
                    </option>
                  ))}
                </select>
