...
export default function ReportMERYRDTPage() {
  ...
  const fetchReportData = useCallback(async (payload: ReportPayload) => {
    ...
    setLoading(true);
    try {
      const reportData = await ReportsService.getMERYRDTReport(currentUserSeid, payload);
      setData(reportData);
    } catch (error) {
      console.error('Error fetching MERYRDT report:', error);
      setError(error instanceof Error ? error.message : 'Failed to fetch MERYRDT report');
      setData([]);
    } finally {
      setLoading(false);
    }
