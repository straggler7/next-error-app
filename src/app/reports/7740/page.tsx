...
export default function Report7740() {
  ...
  const handleRefresh = useCallback(async (payload: ReportPayload) => {
    ...
    setLoading(true);
    try {
      const reportData = await ReportsService.get7740Report(seid, payload);
      setData(reportData);
    } catch (error) {
      console.error('Error fetching 7740 report:', error);
      setError(error instanceof Error ? error.message : 'Failed to fetch 7740 report');
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (payload: ReportPayload, columns: { key: string; label: string; visible: boolean }[]) => {
    ...
    try {
      ...
      // Download as CSV
      ReportsService.downloadCSV(exportData, '7740', columns);
    } catch (error) {
  ...
  return (
    <BaseReport
      title="Error Correction Production Report by Employee (7740 - INC)"
      reportType="7740"
      data={data}
      loading={loading}
