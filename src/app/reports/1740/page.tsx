...
export default function Report1740Page() {
  ...
  const fetchReportData = useCallback(async (payload: ReportPayload) => {
    ...
    setLoading(true);
    try {
      const reportData = await ReportsService.get1740Report(currentUserSeid, payload);
      setData(reportData);
    } catch (error) {
      console.error('Error fetching 1740 report:', error);
      setError(error instanceof Error ? error.message : 'Failed to fetch 1740 report');
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (payload: ReportPayload, columns: { key: string; label: string; visible: boolean }[]) => {
    ...
    try {
      ...
      const exportData = await ReportsService.get1740Report(currentUserSeid, payload);
      
      // Download as CSV
      ReportsService.downloadCSV(exportData, '1740', columns);
    } catch (error) {
      console.error('Error exporting 1740 report:', error);
  ...
  return (
    <BaseReport
      title="New Suspense List (ERS1740 - INC)"
      reportType="1740"
      data={data}
      loading={loading}
