...
export default function Report1340NewPage() {
  ...
  const fetchReportData = useCallback(async (payload: ReportPayload) => {
    ...
    setLoading(true);
    try {
      const reportData = await ReportsService.get1340NewReport(currentUserSeid, payload);
      setData(reportData);
    } catch (error) {
      console.error('Error fetching 1340_New report:', error);
      setError(error instanceof Error ? error.message : 'Failed to fetch 1340_New report');
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (payload: ReportPayload, columns: { key: string; label: string; visible: boolean }[]) => {
    ...
    try {
      ...
      const exportData = await ReportsService.get1340NewReport(currentUserSeid, payload);
      
      // Download as CSV with column configuration
      ReportsService.downloadCSV(exportData, '1340_New', columns);
    } catch (error) {
      console.error('Error exporting 1340_New report:', error);
