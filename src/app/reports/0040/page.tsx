...
export default function Report0040Page() {
  ...
  const fetchReportData = useCallback(async (payload: ReportPayload) => {
    ...
    setLoading(true);
    try {
      const reportData = await ReportsService.get0040Report(currentUserSeid, payload);
      setData(reportData);
    } catch (error) {
      console.error('Error fetching 0040 report:', error);
      setError(error instanceof Error ? error.message : 'Failed to fetch 0040 report');
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = async (payload: ReportPayload, columns: { key: string; label: string; visible: boolean }[]) => {
    ...
    try {
      ...
      const exportData = await ReportsService.get0040Report(currentUserSeid, payload);
      
      // Download as CSV
      ReportsService.downloadCSV(exportData, '0040', columns);
    } catch (error) {
      console.error('Error exporting 0040 report:', error);
  ...
  return (
    <BaseReport
      title="Suspended Records Report"
      reportType="0040"
      data={data}
      loading={loading}
